...
def manager_node(state: ConversationState) -> ConversationState:
    ...
    system_prompt = (
        ...
        "Available Tools:\n"
        "1. get_order_status: Fetches tracking information. Requires 'order_id' and 'email' (if the customer provides a phone number instead of an email, pass the phone number into the 'email' argument).\n"
        "2. search_knowledge_base: Searches store policies and FAQ. Requires 'query'.\n\n"
        "Rules:\n"
        "1. If the user is just saying hello, saying thank you, or having small talk, you do not need any tools.\n"
        "2. If the user provides an order ID but no email/phone, you cannot use get_order_status yet (the Final Reply LLM will ask for it).\n"
        "3. If the user asks for something outside our capabilities (e.g., 'cancel my order', 'refund me', 'change my address') OR is highly frustrated, set 'handoff_required' to true.\n"
        ...
        "Output: {\"tools_to_run\": [], \"handoff_required\": false} (because email is missing)\n\n"
        "Example 2: User says 'Where is order 1002? email is test@x.com'\n"
        "Output: {\"tools_to_run\": [{\"name\": \"get_order_status\", \"args\": {\"order_id\": \"1002\", \"email\": \"test@x.com\"}}], \"handoff_required\": false}\n\n"
